...
| `MARKET_DATA_REQUEST_TIMEOUT` | `15s` | Overall timeout for Kraken, Trading 212, Marketstack, and currency-conversion market-data operations. Maximum is 1 minute. |
| `TRADING212_API_KEY` | empty | Enables read-only stock and ETF account pricing through Trading 212 when configured with the matching secret. |
| `TRADING212_API_SECRET` | empty | Trading 212 API secret. Create a key with portfolio, account data, and history read permissions only. |
| `TRADING212_OWNER_USER_ID` | empty | Required with Trading 212 credentials. Only this backend user ID can invoke the account-specific Trading 212 integration. |
| `TRADING212_BASE_URL` | `https://live.trading212.com/api/v0` | Trading 212 live REST API base URL. |
| `MARKETSTACK_API_KEY` | empty | Enables daily stock and ETF history for the portfolio chart. The key stays on the backend. |
| `MARKETSTACK_BASE_URL` | `https://api.marketstack.com/v2` | Marketstack REST API base URL. |
| `FRANKFURTER_BASE_URL` | `https://api.frankfurter.dev/v1` | ECB-backed daily exchange-rate API used to convert USD stock history to EUR. |
...
BTC and ETH trades accept an EUR amount and exact RFC3339 execution time. Future execution times are rejected. The server fetches a nearby public BTC/EUR or ETH/EUR Kraken trade, records the reference price and provider timestamp, and derives the crypto quantity. Stock and ETF trades use the configured read-only Trading 212 account. Historical entries are matched to the account's own fills, and current valuation uses open positions. Wallet-impact values provide EUR account-currency prices without a separate FX provider. Open positions are cached for five minutes. When Marketstack is configured, its daily end-of-day prices are cached by the backend and included in portfolio history. USD history is converted to EUR with Frankfurter's daily rates. Each history point includes the aggregate portfolio values and a per-holding breakdown for the iOS Portfolio and Holdings chart modes. The Trading 212 credentials remain on the backend and should have no order permissions.

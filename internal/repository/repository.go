...
CREATE TABLE IF NOT EXISTS transactions(id SERIAL PRIMARY KEY,user_id INT NOT NULL REFERENCES users(id) ON DELETE CASCADE,type TEXT NOT NULL,category TEXT NOT NULL,amount NUMERIC(14,2) NOT NULL,currency TEXT NOT NULL,occurred_at DATE NOT NULL);`)
	return err
}
...
func (r *Repository) ListTransactions(ctx context.Context, userID int, month, typ, category string) ([]model.Transaction, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&t.ID, &t.Type, &t.Category, &t.Amount, &t.Currency, &t.OccurredAt); err != nil {
			return nil, err
		}
		out = append(out, t)
	}
